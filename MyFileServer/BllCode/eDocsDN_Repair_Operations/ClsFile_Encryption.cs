...
using R = eDocsDN_Repair_Operations;
    ...
    public class ClsCheck_Configuration_For_File_Storage : ClsFile_Encryption
    {
        ...
        public File_Data Copy_File(Directory_Attributes oSource_Dir, File_Data oSourceFile, Directory_Attributes oDestination_Dir, File_Data oDestination_File)
        {
            ...
            try
            {
                ...
                if (oSource_Dir != null)
                {
                    ...
                    {
                        case true:
                            //if (_bisDebug) File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Physical_Directory: " + oSource_Dir.Physical_Directory.ToString() + Environment.NewLine);
                            ...
                            {
                                case true:
                                    //if (_bisDebug) File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Files_To_Be_Encrypted: " + oSource_Dir.Files_To_Be_Encrypted.ToString() + Environment.NewLine);
                                ...
                                case false:
                                    //if (_bisDebug) File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Files_To_Be_Encrypted: " + oSource_Dir.Files_To_Be_Encrypted.ToString() + Environment.NewLine);
                                    //if (_bisDebug) File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " GetMd5_CheckSum: " + oSource_Dir.Directory_Path + oSourceFile.File_Name + Environment.NewLine);
                            ...
                            break;
                        case false:
                            //..File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Physical_Directory: " + oSource_Dir.Physical_Directory.ToString() + Environment.NewLine);
                            #region .... get file from Database ....
                            _objSave_as_Blob = new ClsSave_File_in_Database(_szAppXmlPath, _szDBName, _szLocation);
                            //..File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Get_File_from_Database: " + Environment.NewLine);
                            oSourceFile = _objSave_as_Blob.Get_File_from_Database(oSource_Dir, oSourceFile);
                            if (_objSave_as_Blob.msgError != "")
                            {
                                //..File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " Error occured on Save operations in Database : " + _objSave_as_Blob.msgError + Environment.NewLine);
                                throw new Exception("Error occured on Save operations in Database : " + _objSave_as_Blob.msgError);
                            }
                            oDestination_File = oSourceFile;
                ...
                {
                    //..File.AppendAllText(oSourceFile.LogFile, DateTime.Now + " oDestination_File = oSourceFile" + Environment.NewLine);
                    oDestination_File = oSourceFile;
                }
                #endregion
      ...
      "Stack trace: " + ex.StackTrace + Environment.NewLine;
            ...
            try
            {
                ...
                if (oSource_Dir.Physical_Directory)
                {
                    ...
                    if (_objINI.IsWordDocument.Contains(Path.GetExtension(szFileName).Replace(".", "").ToUpper()))
                    {
                        if (File.Exists(oSource_Dir.Directory_Path + szFileName))
                        {
                            _objDocPreCheck = new ClsDocumentPre_Check(oSource_Dir.Directory_Path + szFileName);
        ...
        internal void Pre_Check_Document(byte[] arrFile)
        {
            try
            {
                ...
                _objDocPreCheck = new ClsDocumentPre_Check(Convert_Document_To_Stream(arrFile));
                if (!_objDocPreCheck.PreCheck_Document())
                    throw new Exception(_objDocPreCheck.msgError);
     ...
     "Stack trace: " + ex.StackTrace + Environment.NewLine;
        ...
        private File_Data Process_Physical_Document(File_Data oDestinationFile)
        {
            ...
            ClsWord_File_Operations objWord_Operations;
            Update_Document_Custom_Variables objUpdate_Document_Properties = null;
            ...
            try
            {
                ...
                if (_objINI.IsWordDocument.Contains(Path.GetExtension(oDestinationFile.Destination_File_Name).Replace(".", "").ToUpper()))
                {
                    ...
                    if (oDestinationFile.File_Operations != null)
                    {
                        ...
                        {
                            case true:
                                //if (_bisDebug) File.AppendAllText(oDestinationFile.LogFile, DateTime.Now + " Convert To Pdf : " + objFileOperations.ConvertToPdf.ToString() + Environment.NewLine);

                                #region .... Convert To PDF ...
                                //if (_bisDebug) File.AppendAllText(oDestinationFile.LogFile, DateTime.Now + " Destination_File_Name : " + objTempDir.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);
                                File.WriteAllBytes(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name, oDestinationFile.Data);
                                objWord_Operations = new ClsWord_File_Operations(_szAppXmlPath);
                                //..File.AppendAllText(oDestinationFile.LogFile, DateTime.Now + " Convert_DOcument_To_PDF " + Environment.NewLine);

                                if (!objWord_Operations.Convert_DOcument_To_PDF(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name))
                                {
                                    //..File.AppendAllText(oDestinationFile.LogFile, DateTime.Now + " Error Occured while Converting File To PDF : " + objWord_Operations.msgError + Environment.NewLine);
                            ...
                            default:
                                ...
                                File.AppendAllText(_szLogFileName, DateTime.Now + " : Process_Physical_Document" + Environment.NewLine);
                                ...
                                if (objFileOperations.Update_Properties != null)
                                {
                                    ...
                                    switch (objUpdate_Document_Properties.eDocument_Process)
                                    {
                                        case Process.Repair_Document:
                                            ...
                                            File.AppendAllText(_szLogFileName, DateTime.Now + " : objUpdate_Document_Properties.eDocument_Process" + objUpdate_Document_Properties.eDocument_Process.ToString() + Environment.NewLine);
                    ...
                    #region .... Return Process File ....
                    File.AppendAllText(_szLogFileName, DateTime.Now + " Convert File To Stream : " + objTempDir.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);
                    if (File.Exists(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name))
                    {
                        WaitForFile(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name);
                        File.AppendAllText(_szLogFileName, DateTime.Now + " Read File : " + objTempDir.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);
                        ...
                        File.AppendAllText(_szLogFileName, DateTime.Now + " CheckSum : " + oDestinationFile.CheckSum + Environment.NewLine);
                        oDestinationFile.Source_File_CheckSum = oDestinationFile.CheckSum;
                    }
                    File.AppendAllText(_szLogFileName, DateTime.Now + " : Converted Physical File to Stream" + Environment.NewLine);

                    #endregion

                }
                File.AppendAllText(_szLogFileName, DateTime.Now + " : END : " + _szAppXmlPath + Environment.NewLine);
            ...
            {
                File.AppendAllText(_szLogFileName, DateTime.Now + " : Delete physical File" + Environment.NewLine);
                if (File.Exists(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name))
                {
                    WaitForFile(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name);
                    File.Delete(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name);
                ...
                File.AppendAllText(_szLogFileName, DateTime.Now + " : Deleted physical File" + Environment.NewLine);
                                ...
                                if (objFileOperations.Update_Properties != null)
                                {
                                    ...
                                    switch (objUpdate_Document_Properties.eDocument_Process)
                                    {
                                        ...
                                        case Documents_Process.Controller_Publish:
                                            ...
                                            File.AppendAllText(_szLogFileName, DateTime.Now + " : objUpdated Document_Properties.eDocument_Process  " + Environment.NewLine);

                                            break;
                                        case Documents_Process.Preview:
                                            File.AppendAllText(_szLogFileName, DateTime.Now + " Preview Process " + Environment.NewLine);
                                            ...
                                            File.AppendAllText(_szLogFileName, DateTime.Now + " Previewed Process " + Environment.NewLine);
                                            break;
                                        default:
                                            break;
                                    }
