...
namespace eDocsDN_File_Operations
{
    public class ClsCheck_Configuration_For_File_Storage : ClsFile_Encryption
    {
        ...
        public ClsCheck_Configuration_For_File_Storage(string szAppXmlPath, string szDBName, string szLocation)
        {
            ...
            iTimeOut = Convert.ToInt32(_objINI.GetLocationVariable(szLocation, "", "TimeOut"));
        ...
        public bool Check_File_is_Locked(Directory_Attributes oSource_Dir, string szFileName)
        {
            ...
            try
            {
                if (oSource_Dir.Physical_Directory)
                {
                    DateTime now1;
                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " check File Exist " + Environment.NewLine);
                    if (System.IO.File.Exists(oSource_Dir.Directory_Path + szFileName))
                    {
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " check File Exist " + oSource_Dir.Directory_Path + szFileName + Environment.NewLine);
                        using (_objINI = new clsReadAppXml(_szAppXmlPath))
                            uriString = _objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "DocServer").Trim();

                        if (IsLocalPath(uriString))
                        {
                            using (StreamReader reader = new StreamReader(oSource_Dir.Directory_Path + szFileName))
                            {
                                string line;
                                // Read line by line  
                        ...
                        {
                            DateTime now2;
                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " uriString " + uriString + Environment.NewLine);
                            ...
                            using (WebDavClient webDavClient = new WebDavClient(new WebDavClientParams()
                            {
                                BaseAddress = new Uri(uriString)

                            }))
                            {
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " BaseAddress " + Environment.NewLine);
                                PropfindResponse result = webDavClient.Propfind(szFileName).Result;
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " result " + result.ToString() + Environment.NewLine);
                                int count = ((List<WebDavResource>)result.Resources)[0].ActiveLocks.Count;
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " count " + count + Environment.NewLine);
                                List<WebDavProperty> properties = (List<WebDavProperty>)((List<WebDavResource>)result.Resources)[0].Properties;
                                if (!string.IsNullOrEmpty(properties[2].Value))
                                {

                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " iActiveLock1[2].Value " + properties[2].Value + Environment.NewLine);
                                    if (!string.IsNullOrEmpty(properties[2].Value))
                                    {
                                        XDocument xdocument = XDocument.Parse(properties[2].Value);
                                        XNamespace xnamespace = (XNamespace)"DAV:";
                                        string str = xdocument.Root.Element(xnamespace + "owner").Element(xnamespace + "href").Value.Trim();
                                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " enter " + str + Environment.NewLine);
                                        flag = true;
                                    }
                                    else
                                    {
        ...
        public bool Check_File_is_Locked(Directory_Attributes oSource_Dir, string szFileName, string szOfficeVersion)
        {
            bool flag = false;
            string uriString = string.Empty;
            try
                ...
                if (oSource_Dir.Physical_Directory)
                {
                    ...
                    if (System.IO.File.Exists(oSource_Dir.Directory_Path + szFileName))
                    {
                        ...
                        using (_objINI = new clsReadAppXml(_szAppXmlPath))
                        {
                            ...
                            switch (szOfficeVersion)
                            {
                                case "365":
                                    uriString = _objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "DocServer").Trim();
                                    break;
                                default:
                                    uriString = _objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "DocServerOther").Trim();
                                    break;
                    ...
                    case false:
                        switch (oSource_Dir.Files_To_Be_Encrypted)
                        {
                            ...
                            case false:
                                ...
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "Path " + oSource_Dir.Directory_Path + oSourceFile.File_Name + Environment.NewLine);
                                if (File.Exists(oSource_Dir.Directory_Path + oSourceFile.File_Name))
                                {
                                ...
                                {
                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " File Not Found " + oSource_Dir.Directory_Path + oSourceFile.File_Name + Environment.NewLine);
                                }
      ...
      "Stack trace: " + ex.StackTrace + Environment.NewLine;
        ...
        public bool Convert_To_PDF(Directory_Attributes oSource_Dir, File_Data oSourceFile, Directory_Attributes oDestination_Dir, File_Data oDestination_File)
        {
            ...
            try
            {
                ...
                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "Process_Document_Stream" + Environment.NewLine);
                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "oDestination_Dir.Directory_Path + oDestination_File.Destination_File_Name" + oDestination_Dir.Directory_Path + oDestination_File.Destination_File_Name + Environment.NewLine);

                objWord_Operations = new ClsXml_Operations(_szAppXmlPath);
                if (!objWord_Operations.Convert_Document_To_PDF(oDestination_Dir.Directory_Path + oDestination_File.Destination_File_Name))
                {
...
"Stack trace: " + ex.StackTrace + Environment.NewLine;
        ...
        private File_Data Process_Document_Stream(File_Data oSourceFile)
        {
            ...
            eDocDN_Add_Signatory_Page.ClseAdd_Signatory_Page objSign = null;
                            ...
                            default:
                                ...
                                if (objFileOperations.Update_Properties != null)
                                {
                                    ...
                                    if (objFileOperations.Update_Properties.eDocument_Process == Documents_Process.Controller_Live)
                                    {
                                        ...
                                        //if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Remove Signtory Page" + Environment.NewLine);
                                        //objSign = new eDocDN_Add_Signatory_Page.ClseAdd_Signatory_Page(_szDBName, _szAppXmlPath, strmDocument);
                                        //strmDocument = objSign.Delete_Signatures(oSourceFile.SurrKey);
                                        //if (objSign.msgError != "")
                                        //    throw new Exception("Error occured while Deleting Signatory Page. Error :" + objSign.msgError);

                                        //if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Removed Signtory Page" + Environment.NewLine);
                                ...
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Set Print Form Data " + oSourceFile.PrintFormData + Environment.NewLine);
                                ...
                                #region .... Add Dynamic Signatore ....
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Add Dynamic Signatore" + Environment.NewLine);
                                if (objFileOperations != null)
                                {
                                    if (objFileOperations.Update_Properties != null)
                                    {
                                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Remove Dynamic Signatore" + Environment.NewLine);
                                        switch (objFileOperations.Update_Properties.eDocument_Process)
                                        {
                                            case Documents_Process.Controller_Live:
                                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Remove Dynamic Signatore" + objFileOperations.Update_Properties.eDocument_Process.ToString() + Environment.NewLine);
                                                objSign = new eDocDN_Add_Signatory_Page.ClseAdd_Signatory_Page(_szDBName, _szAppXmlPath, strmDocument);
                                                strmDocument = objSign.Delete_Signatures(oSourceFile.SurrKey);
                                                if (objSign.msgError != "")
                                                    throw new Exception("Error occured while Deleting Signatory Page. Error :" + objSign.msgError);
                                            ...
                                            case Documents_Process.Preview:
                                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Add Dynamic Signatore" + objFileOperations.Update_Properties.eDocument_Process.ToString() + Environment.NewLine);

                                                objSign = new eDocDN_Add_Signatory_Page.ClseAdd_Signatory_Page(_szDBName, _szAppXmlPath, strmDocument);
                                                objSign.eDocumentProcess = objFileOperations.Update_Properties.eDocument_Process;
                                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Calling Function " + Environment.NewLine);
                                                if (!objSign.Add_Signatory_Page(oSourceFile.SurrKey))
                                                    throw new Exception("Error occured while Deleting Signatory Page. Error :" + objSign.msgError);
                                ...
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : END Dynamic Signatore" + Environment.NewLine);
                                #endregion

                                #region .... Track Changes ...
                                if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Set TrackRevisions" + oSourceFile.TrackChanges.ToString() + Environment.NewLine);
     ...
     "Stack trace: " + ex.StackTrace + Environment.NewLine;
        ...
        private File_Data Process_Document_Physically(File_Data oSourceFile)
        {
            ...
            using (_objINI = new clsReadAppXml(_szAppXmlPath))
            {
                switch (_objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "PeerToPeer"))
                {
                    case "1":
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "PeerToPeer" + Environment.NewLine);
                        objTempDir = objDir.GetDirPath("SP", false, true, false);
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "PeerToPeer Path" + objTempDir.Directory_Path + Environment.NewLine);
                        break;
                    default:
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "PeerToPeer False" + Environment.NewLine);

                        objTempDir = objDir.GetDirPath("TS", false, true, false);
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + "Path" + objTempDir.Directory_Path + Environment.NewLine);
                        break;
                }
            }
            ...
            try
            {
                ...
                if (_objINI.IsWordDocument.Contains(Path.GetExtension(oSourceFile.File_Name).Replace(".", "").ToUpper()))
                {
                    ...
                    if (oSourceFile.File_Operations != null)
                    {
                        ...
                        #region .... Print Form Data ...
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Set Print Form Data " + oSourceFile.PrintFormData.ToString() + Environment.NewLine);
                        ...
                        if (objFileOperations != null)
                        {
                            if (objFileOperations.Update_Properties != null)
                            {
                                switch (objFileOperations.Update_Properties.eDocument_Process)
                                {
                                    case Documents_Process.Controller_Live:
                                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Remove Signtory Page" + Environment.NewLine);
                                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Remove Signtory Page" + objTempDir.Directory_Path + oSourceFile.Destination_File_Name + Environment.NewLine);
                                        objSign = new eDocDN_Add_Signatory_Page.ClseAdd_Signatory_Page(_szDBName, _szAppXmlPath, objTempDir.Directory_Path + oSourceFile.Destination_File_Name);
                                        objSign.Delete_Signatures(oSourceFile.SurrKey);
                                        if (objSign.msgError != "")
                                            throw new Exception("Error occured while Deleting Signatory Page. Error :" + objSign.msgError);
                        ...
                        if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : End of Dynamic Signatore" + Environment.NewLine);
            ...
            finally
            {
                ...
                if (objTempDir != null)
                    if (File.Exists(objTempDir.Directory_Path + oSourceFile.Destination_File_Name))
                    {
                        WaitForFile(objTempDir.Directory_Path + oSourceFile.Destination_File_Name);
                        File.Delete(objTempDir.Directory_Path + oSourceFile.Destination_File_Name);
                        if (oSourceFile.Source_Directory.Equals("MI", StringComparison.InvariantCultureIgnoreCase))
                        {
                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " (Migration) " + Environment.NewLine);
                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " oSourceFile.File_Name " + oSourceFile.File_Name + Environment.NewLine);
                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " oSourceFile.Destination_File_Name " + oSourceFile.Destination_File_Name + Environment.NewLine);
                            oSourceFile.File_Name = oSourceFile.Destination_File_Name;
                                ...
                                if (objFileOperations.Update_Properties != null)
                                {
                                    ...
                                    switch (objUpdate_Document_Properties.eDocument_Process)
                                    {
                                        ...
                                        case Documents_Process.Controller_Publish:
                                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Write file Physically : " + objTempDir.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);

                                            File.WriteAllBytes(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name, oDestinationFile.Data);

                                            switch (_isPeerToPeer)
                                            {
                                                case true:
                                                    Directory_Attributes objTempDirPath = objDir.GetDirPath("SP", false, true, false);//..SharedPath
                                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : Pass Shared Path to Windows Service : " + objTempDirPath.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);
                                                    using (eDocsDN_Word_File_Operations.ClsWord_File_Operations objProcess = new eDocsDN_Word_File_Operations.ClsWord_File_Operations(_szAppXmlPath))
                                                    {
                                                        if (!objProcess.Update_Document_Properties(objTempDirPath.Directory_Path + oDestinationFile.Destination_File_Name))
                                                        {
                                                            throw new Exception("Error Occured while Processing Document(Physical). Error : " + objProcess.msgError + Environment.NewLine);
                                                        }
                                                    }
                                                    objTempDirPath = null;
                                        ...
                                        case Documents_Process.Preview:
                                            ...
                                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : WriteAllBytes " + objTempDir.Directory_Path + oDestinationFile.Destination_File_Name + Environment.NewLine);
                                            File.WriteAllBytes(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name, oDestinationFile.Data);
                                            ...
                                            if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : TimeOut " + iTimeOut.ToString() + Environment.NewLine);
                                            ...
                                            switch (_isPeerToPeer)
                                            {
                                                case true:
                                                    ...
                                                    using (eDocsDN_Word_File_Operations.ClsWord_File_Operations objProcess = new eDocsDN_Word_File_Operations.ClsWord_File_Operations(_szAppXmlPath))
                                                    {
                                                        if (!objProcess.Process_Word_Document(objTempDirPath.Directory_Path + oDestinationFile.Destination_File_Name))
                                                        {
                                                            throw new Exception("Error Occured while Processing Document(Physical). Error : " + objProcess.msgError + Environment.NewLine);
                                                        }
                                                    }
                                                    objTempDirPath = null;
                                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : END of objWord_Operations " + Environment.NewLine);
                                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : objUpdate_Document_Properties.eDocument_Process " + objUpdate_Document_Properties.eDocument_Process.ToString() + Environment.NewLine);
                                                    break;
                                                default:
                                                    objWord_Operations = new ClsXml_Operations(_szAppXmlPath);
                                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : objWord_Operations " + Environment.NewLine);
                                                    var task = Task.Run(() => objWord_Operations.Process_Word_Document(objTempDir.Directory_Path + oDestinationFile.Destination_File_Name));
                                                    if (task.Wait(TimeSpan.FromSeconds(iTimeOut)))
                                                    {
                                                        if (objWord_Operations.msgError != "")
                                                            throw new Exception(objWord_Operations.msgError);
                                                    }
                                                    else
                                        ...
                                        case Documents_Process.Controller_Publish:
                                            ...
                                            switch (_objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "PeerToPeer"))
                                            {
                                                case "1":
                                                    using (eDocsDN_Word_File_Operations.ClsWord_File_Operations objProcess = new eDocsDN_Word_File_Operations.ClsWord_File_Operations(_szAppXmlPath))
                                                    {
                                                        if (!objProcess.Update_Document_Properties(szFilePath))
                                                        {
                                                            throw new Exception("Error Occured while Processing Document(Physical). Error : " + objProcess.msgError + Environment.NewLine);
                                                        }
                                                    }
                                        ...
                                        case Documents_Process.Preview:
                                            ...
                                            switch (_objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "PeerToPeer"))
                                            {
                                                ...
                                                default:
                                                    objWord_Operations = new ClsXml_Operations(_szAppXmlPath);
                                                    if (_bisDebug) File.AppendAllText(_szLogFileName, DateTime.Now + " : objWord_Operations " + Environment.NewLine);
                                                    var task = Task.Run(() => objWord_Operations.Process_Word_Document(szFilePath));
                                                    if (task.Wait(TimeSpan.FromSeconds(iTimeOut)))
                                                    {
                                                        if (objWord_Operations.msgError != "")
                                                            throw new Exception(objWord_Operations.msgError);
        ...
        internal void File_Storage_in_Database()
        {
            ...
            try
            {
                ...
                _isPeerToPeer = _objINI.GetLocationVariable(_objINI.GetCurrentLocation(), "", "PeerToPeer").ToUpper() == "1" ? true : false;
                switch (szFileSystem)
                {
                    case "0": //..Physical
