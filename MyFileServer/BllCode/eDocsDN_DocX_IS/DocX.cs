...
namespace eDocsDN_DocX_IS
{
    ...
    public class DocX : Container, IDisposable
    {
        ...
        private List<Header> GetHeaderByType(string type)
        {
            return GetHeaderByType(type, true);
        }

        private List<Footer> GetFooterByType(string type)
        {
            return GetFooterByType(type, false);
        }
        ...
        private object GetHeaderOrFooterByType(string type, bool isHeader)
        {
            ...
            if (Id != null)
            {
                ...
                using (TextReader tr = new StreamReader(part.GetStream()))
                {
                    ...
                    Container c;
                    if (isHeader)
                        c = new Header(this, doc.Element(w + "hdr"), part, Id);
                    else
                        c = new Footer(this, doc.Element(w + "ftr"), part, Id);
        ...
        private List<Header> GetHeaderByType(string type, bool isHeader)
        {
            // Switch which handles either case Header\Footer, this just cuts down on code duplication.
            List<Header> H = null;
            //List<Footer> F = null;
                ...
                if (Id != null)
                {
                    ...
                    using (TextReader tr = new StreamReader(part.GetStream()))
                    {
                        ...
                        if (isHeader)
                        {
                            ...
                            H.Add(new Header(this, doc.Element(w + "hdr"), part, Id));
                        }
                        //else
                        //{
                        //    if (F == null)
                        //        F = new List<Footer>();
                        //    F.Add(new Footer(this, doc.Element(w + "ftr"), part));
                        //}
                    }
                }
            }
        ...
        private List<Footer> GetFooterByType(string type, bool isHeader)
        {
            // Switch which handles either case Header\Footer, this just cuts down on code duplication.
            //List<Header> H = null;
            List<Footer> F = null;
                ...
                if (Id != null)
                {
                    ...
                    using (TextReader tr = new StreamReader(part.GetStream()))
                    {
                        ...
                        if (isHeader)
                        {
                            ...
                            //    H.Add(new Header(this, doc.Element(w + "hdr"), part));
                        }
                        else
                        {
                            if (F == null)
                                F = new List<Footer>();
                            F.Add(new Footer(this, doc.Element(w + "ftr"), part, Id));
                        }
                    }
                }
            }
        ...
        private static void PopulateDocument(DocX document, Package package)
        {
            ...
            headers.lstodd = document.GetHeaderByType("default");
            headers.lsteven = document.GetHeaderByType("even");
            headers.lstfirst = document.GetHeaderByType("first");

            Footers footers = new Footers();
            footers.lstodd = document.GetFooterByType("default");
            footers.lsteven = document.GetFooterByType("even");
            footers.lstfirst = document.GetFooterByType("first");
                ...
                switch (rel.RelationshipType)
                {
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/endnotes":
                        ...
                        using (TextReader tr = new StreamReader(document.endnotesPart.GetStream()))
                            document.endnotes = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
                    ...
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/footnotes":
                        ...
                        using (TextReader tr = new StreamReader(document.footnotesPart.GetStream()))
                            document.footnotes = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
                    ...
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/styles":
                        ...
                        using (TextReader tr = new StreamReader(document.stylesPart.GetStream()))
                            document.styles = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
                    ...
                    case "http://schemas.microsoft.com/office/2007/relationships/stylesWithEffects":
                        ...
                        using (TextReader tr = new StreamReader(document.stylesWithEffectsPart.GetStream()))
                            document.stylesWithEffects = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
                    ...
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/fontTable":
                        ...
                        using (TextReader tr = new StreamReader(document.fontTablePart.GetStream()))
                            document.fontTable = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
                    ...
                    case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/numbering":
                        ...
                        using (TextReader tr = new StreamReader(document.numberingPart.GetStream()))
                            document.numbering = XDocument.Load(tr, LoadOptions.PreserveWhitespace);
                        break;
        ...
        public void Save()
        {
            ...
            if (sectPr != null)
            {
                ...
                foreach (var evenHeaderRef in lstevenHeaderRef)
                {
                    if (evenHeaderRef != null)
                    {
                        //XElement even = headers.even.Xml;
                        foreach (var ieven in headers.lsteven)
                        {

                            if (ieven.HeaderID == evenHeaderRef)
                            {
                                XElement even = ieven.Xml;
                ...
                foreach (var oddHeaderRef in lstoddHeaderRef)
                {
                    if (oddHeaderRef != null)
                    {
                        foreach (var iHeder in headers.lstodd)
                        {

                            if (iHeder.HeaderID == oddHeaderRef)
                            {
                                XElement odd = iHeder.Xml;
                                Uri target = PackUriHelper.ResolvePartUri
                                (
                                    mainPart.Uri,
                ...
                foreach (var firstHeaderRef in lstfirstHeaderRef)
                {
                    if (firstHeaderRef != null)
                    {

                        foreach (var ifirst in headers.lstfirst)
                        {
                            if (ifirst.HeaderID == firstHeaderRef)
                            {
                                XElement first = ifirst.Xml;
                                Uri target = PackUriHelper.ResolvePartUri
                                (
                                    mainPart.Uri,
                ...
                foreach (var oddFooterRef in lstoddFooterRef)
                {
                    if (oddFooterRef != null)
                    {
                        foreach (var ifooters in footers.lstodd)
                        {
                            if (ifooters.FooterID == oddFooterRef)
                            {
                                XElement odd = ifooters.Xml;
                                Uri target = PackUriHelper.ResolvePartUri
                                (
                                    mainPart.Uri,
                ...
                foreach (var evenFooterRef in lstevenFooterRef)
                {
                    if (evenFooterRef != null)
                    {
                        foreach (var ieven in footers.lsteven)
                        {
                            if (ieven.FooterID == evenFooterRef)
                            {
                                XElement even = ieven.Xml;
                                Uri target = PackUriHelper.ResolvePartUri
                                (
                ...
                foreach (var firstFooterRef in lstfirstFooterRef)
                {
                    if (firstFooterRef != null)
                    {
                        foreach (var ifooters in footers.lstfirst)
                        {
                            if (ifooters.FooterID == firstFooterRef)
                            {
                                XElement first = ifooters.Xml;
                                Uri target = PackUriHelper.ResolvePartUri
                                (
                                    mainPart.Uri,
        ...
        internal static void UpdateCorePropertyValue(DocX document, string corePropertyName, string corePropertyValue)
        {
            ...
            foreach (XElement e in document.mainDoc.Descendants(XName.Get("fldSimple", w.NamespaceName)))
            {
                ...
                if (Regex.IsMatch(attr_value, matchPattern))
                {
                    ...
                    eDocsDN_DocX_IS.Text.PreserveSpace(t);
        ...
        /// <seealso cref="CustomProperty"/>
        /// <seealso cref="CustomProperties"/>
        public void Attach_Custom_Property(CustomProperty cp)
        {
            // If this document does not contain a customFilePropertyPart create one.
            if (!package.PartExists(new Uri("/docProps/custom.xml", UriKind.Relative)))
                HelperFunctions.CreateCustomPropertiesPart(this);
        ...
        public List<XElement> Get_document_xElement(DocX document)
        {
            ...
            #region Headers
            Headers headers = document.Headers;
            if (headers.lstfirst != null) { foreach (var item in headers.lstfirst) { documents.Add(item.Xml); } }
            if (headers.lstodd != null) { foreach (var item in headers.lstodd) { documents.Add(item.Xml); } }
            if (headers.lsteven != null) { foreach (var item in headers.lsteven) { documents.Add(item.Xml); } }
            #endregion
            ...
            if (footers.lstfirst != null) { foreach (var item in footers.lstfirst) { documents.Add(item.Xml); } }
            if (footers.lstodd != null) { foreach (var item in footers.lstodd) { documents.Add(item.Xml); } }
            if (footers.lsteven != null) { foreach (var item in footers.lsteven) { documents.Add(item.Xml); } }
        ...
        public void UpdateScanSign(DocX document, Dictionary<string, string> ScanSignOfUser, bool bRemoveScanSign)
        {
            msgError = "";
            try
            {
        ...
        internal static void UpdateCustomPropertyValue(DocX document, string customPropertyName, string customPropertyValue)
        {
            ...
            StringBuilder sbCustVariable = null;
            List<XElement> documents = new List<XElement> { document.mainDoc.Root };
            try
            {
                #region ... Headers ....
                ...
                foreach (XElement doc in documents)
                {
                    ...
                    foreach (XElement e in doc.Descendants(XName.Get("instrText", w.NamespaceName)))
                    {
                        ...
                        string match_value = string.Format(@"DOCPROPERTY  {0}  \* MERGEFORMAT", customPropertyName).Replace("DOCPROPERTY", "").Replace(@"\* MERGEFORMAT", "").Replace("\"", "").Replace("MERGEFIELD", "").Trim();
                        if (!(e.Value.Contains("DOCPROPERTY") && e.Value.Contains("MERGEFORMAT")))
                        {
                            #region ... For Currupted Tags ....
                            sbCustVariable = new StringBuilder();
                            var qryRecords = from record in e.Parent.Parent.Descendants((XName.Get("instrText", w.NamespaceName))) select record;
                            foreach (var item in qryRecords) { sbCustVariable.Append(item.Value); }
                            attr_value = sbCustVariable.ToString().Replace("DOCPROPERTY", "").Replace(@"\* MERGEFORMAT", "").Replace("\"", "").Replace("MERGEFIELD", "").Trim();
                            sbCustVariable = null;
                            qryRecords = null;
                            #endregion
                        }
                        ...
                        {
                            attr_value = e.Value.Replace("DOCPROPERTY", "").Replace(@"\* MERGEFORMAT", "").Replace("\"", "").Replace("MERGEFIELD", "").Trim();
                        }
                        attr_value = rgx.Replace(attr_value, "");
                        attr_value = Regex.Replace(attr_value, @"\s+", "");
                        match_value = Regex.Replace(match_value, @"\s+", "");
                    ...
                    {
                        string match_value = string.Format(@"DOCPROPERTY  {0}  \* MERGEFORMAT", customPropertyName).Replace("DOCPROPERTY", "").Replace(@"\* MERGEFORMAT", "").Replace("\"", "").Replace("MERGEFIELD", "").Trim();
                        string attr_value = e.Attribute(XName.Get("instr", w.NamespaceName)).Value.Trim().Replace("DOCPROPERTY", "").Replace(@"\* MERGEFORMAT", "").Replace("\"", "").Replace("MERGEFIELD", "").Trim();
                        attr_value = rgx.Replace(attr_value, "");
                        attr_value = Regex.Replace(attr_value, @"\s+", "");
                        match_value = Regex.Replace(match_value, @"\s+", "");
